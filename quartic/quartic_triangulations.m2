...
out = openOut "quartic_triangulations.csv";
...
for tr in keys(atht) do (
    if atht#tr then (
        ...
        usedindices = unique sort flatten tr;
        invindices = hashTable apply(length(usedindices), i -> (usedindices_i, i));

        out << toString (Apoints_(unique sort flatten tr))
            << toString apply(tr, t -> t / (v -> invindices#v + 1))
            << toString ht << endl << flush;
    );
);
